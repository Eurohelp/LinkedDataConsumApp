...
public class Stardog {
	...
	public String getDatosNumVentasTotalesEnTodasLasSedesResource(String resourceName) throws RepositoryException, MalformedQueryException, QueryEvaluationException{
		String result = "";
	...
	public String getDatosNumVentasTotalesEnSedeConcretaResource(String resourceName, String sede) throws RepositoryException, MalformedQueryException, QueryEvaluationException{
		String result = "";
		String queryText = "SELECT ?labelRecurso ?labelSede (SUM(?numVentas) as ?numTotal) where{ graph<http://lod.eurohelp.es/graph/novelas>{?recursoAutorDbpedia <http://schema.org/author> ?recurso. ?datosVenta <http://lod.eurohelp.es/def/product> ?recurso. ?recurso <http://schema.org/name> ?labelRecurso. ?sede ?seVendio ?datosVenta. ?sede <http://www.w3.org/2000/01/rdf-schema#label> ?labelSede. ?datosVenta <http://dbpedia.org/ontology/numberSold> ?numVentas.}FILTER (?labelRecurso IN ("+resourceName+"^^xsd:string)) FILTER (?labelSede IN ("+sede+"))}GROUP BY ?labelRecurso ?recurso ?labelSede";
				System.out.println("Query obtiene nombre recurso-->" + queryText);
		TupleQuery query = repository.prepareTupleQuery(QueryLanguage.SPARQL, queryText);
		TupleQueryResult queryResults = query.evaluate();
		while (queryResults.hasNext()) {
